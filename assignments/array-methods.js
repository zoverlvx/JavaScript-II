...
let allCaps = [];
const caps = runners.map(function (runner) {
...
let largeShirts = [];
const reorderFor = runners.filter(function (runner) {
    return runner.shirt_size === "L";
});
largeShirts = [...reorderFor];
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = [];
const reducer = (a, c) => a + c.donation;
const v = runners.reduce(reducer, 0);
ticketPriceTotal.push(v);
...
// Problem 1
const removedLastNames = runners.forEach(function (visitor) {
    delete visitor.last_name;
});
console.log(removedLastNames);
// Problem 2
const nameTagged = runners.map(function (visitor) {
    visitor.name_tag = visitor.first_name;
    return visitor;
});
...
function searchByCompany (companyName) {
    const found = runners.filter(function (runner) {
        return runner.company_name === companyName;
    });
    return found;
}
console.log(searchByCompany("Zoonoodle"));
